...
func Load() Config {
	...
	return Config{
		...
		YandexRedirectURI:     env("YANDEX_REDIRECT_URI", env("REDIRECT_URI", "http://localhost:8081/auth/callback")),
		KeycloakAdminUser:     env("KEYCLOAK_ADMIN", "admin"),
		KeycloakAdminPassword: env("KEYCLOAK_ADMIN_PASSWORD", "admin"),
		KeycloakProvisionRole: env("KEYCLOAK_PROVISION_ROLE", "prothetic_user"),
	}
}
